...
plt.xlabel('Płeć')
plt.subplot(1, 3, 2)
plt.plot(df.Rok.unique(), [ch.values[y][0]
                           for y in range(len(ch.values))], "r", label="M")
plt.plot(df.Rok.unique(), [dz.values[y][0]
                           for y in range(len(dz.values))], "g", label="K")
plt.xticks(df.Rok.unique(), rotation=60)
plt.ylabel('Ilość urodzeń')
plt.xlabel('Rok')
plt.legend()
plt.subplot(1, 3, 3)
plt.bar(df.Rok.unique(), [p2.values[y][0]
                          for y in range(len(p2.values))], color="pink")
plt.xticks(df.Rok.unique(), rotation=60)
plt.ylabel('Ilość urodzeń')
plt.xlabel('Rok')
plt.show()
